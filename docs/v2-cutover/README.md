...
향후 핵심 화면, 개발 추적표를 한 화면에서 보여 준다. 보드에서 실제 선택지와 수정·설명 메모를 작성한 뒤
...
1. [시각 검토 보드](review-board/index.html)의 `실행 로드맵`을 먼저 연다. 하루 두 번의
   QQQM/QLD/TQQQ 판단, C2 개발 6묶음, 코드 완료 뒤에도 남는 C3~C7 순서를 확인한다.
2. `결정 10가지`를 연다. 항상 하나의 결정만
   펼쳐지며, 접힌 줄의 `미응답/선택` 표시로 진행 상태를 확인한다.
3. 각 D-01~D-10에서 문서와 1:1로 맞춘 실제 선택지 하나를 고른다. 선택만으로는
   화면이 움직이지 않는다. 수정 또는 설명을 고르면 필수 메모를 작성한다.
4. `다음 미응답 보기`가 활성화된 뒤 직접 눌러 다음 결정으로 간다. D-07은 향후 QA
   범위·보존 기준만, D-09·10은 미래 복구·자동운용 완료 기준만 정하며 실행 승인이 아니다.
5. `10 / 10 응답 작성`과 `검토안이 준비됐어요`를 확인한 뒤 `검토안 복사`를 누르고,
   `D-01 ... D-10 ...` 검토안을 이 Codex 작업에 붙여넣는다.
6. 반영된 [C0-A 기록](C0_DECISIONS.md#c0-a-제품-결정-기록)을 다시 확인한다. 이후에도
...
| 궁금한 점 | 문서 | 진행 권한 |
| --- | --- | --- |
| 승인 뒤 무엇을 어떤 순서로 구현하고 검증하는지 | [IMPLEMENTATION_RUNWAY.md](IMPLEMENTATION_RUNWAY.md) | C0-B 결과 재승인 전에는 기획 검토만 가능 |
