...
- 현재 기획·안전 검증 source 정본: `46a34eaa9251637a117d038a6eb62c51263fff65`. 원본을 checkout한
  ...
  [QA-PLN-001 정본 보고서](evidence/2026-08-05-QA-PLN-001.md)에 기록했다.
...
- 기능 후보와 `a6a7174`의 과거 실행은 각 기능을 도입했을 당시의 이력으로만 남긴다.
  현재 전체 회귀·UI 증적은 위 `46a34ea` 원본 push와 `992b403` test-merge PR 실행이다.
...
| D-07 | `V2-QA-001` | QA 기록 묶음·정리 tombstone·후보 전용 snapshot 기반 GET-only 배포 화면 harness | `S-03`, `S-08` | `QA-NAV-001`, `QA-OPS-001`, `QA-RWD-001`, `QA-SAF-001`: 로컬 가상 데이터 화면·증적 경계만 검증. 후속 후보의 `QA-ACC-002`는 exact health+snapshot 2개 backend GET, 5개 화면 API local fulfill/backend continue 0, 6화면×2 viewport의 PNG 12+관찰 JSON 2+manifest 1 계약만 로컬 검증했으며 실제 인증 화면은 미실행. 상태를 바꾸는 `QA-OPS-002`, `QA-MAN-001~009`도 미실행 | 현재 source `46a34ea`: [push CI 30975237262](https://github.com/ksi2564/systematic-trading/actions/runs/30975237262), test-merge `992b403`: [PR CI 30975239146](https://github.com/ksi2564/systematic-trading/actions/runs/30975239146)의 `ui-qa` 및 artifact PASS. 26/26·증적 참조 78개 SHA-256·리소스 변경 0을 다운로드 후 재검증했다. `MANUAL` 9·`BLOCKED` 2·`OUT_OF_SCOPE` 1은 통과로 올리지 않음 | [QA-PLN-001 정본 증적](evidence/2026-08-05-QA-PLN-001.md) · [자동·수동 QA 경계](TRACEABILITY_QA.md#4-사용자에게-남길-수동-전용-시나리오) | 부분 | GET-only 경로: 후보 배포 승인 → 사용자 직접 로그인 → 자동 조회 QA / 상태변경 경로: 대상·영향·정리 방법 확인 → 별도 실행 승인 |
...
| C1 현재 UI | `V2-UI-001`, `V2-OPS-001` | 현재 6개 화면, 안전 상태 `checking/verified/unsafe/unavailable` | 현재 운영 콘솔(S-01 일부·S-02·S-03·S-05·S-06·S-07) | `QA-NAV-001`, `QA-OPS-001`, `QA-SAF-001`, `QA-RWD-001`: **현재 구현 화면** desktop/mobile mock 후보. 배포 성공 bundle 계약은 6화면×2 viewport, PNG 12+관찰 JSON 2+manifest 1의 정확한 15파일 | 현재 source `46a34ea`: [push CI 30975237262](https://github.com/ksi2564/systematic-trading/actions/runs/30975237262), test-merge `992b403`: [PR CI 30975239146](https://github.com/ksi2564/systematic-trading/actions/runs/30975239146)의 `frontend`·`ui-qa` 26/26 PASS. 운영 배포·인증 화면은 미실행 | [QA-PLN-001 정본 증적](evidence/2026-08-05-QA-PLN-001.md) · [현재 UI QA 범위](TRACEABILITY_QA.md) | 부분 | 사용자 승인 동일 SHA 배포 → 인증 QA |
...
| 공통 명세 | `V2-DOC-001` | 요구·기능·화면·QA·롤백·기능 제안 템플릿, C0 기록 완전성 gate | **기획 미리보기** `S-01/S-04/S-08/S-10/S-09` | `QA-PLN-001`: 실제 선택지 35개·한 번에 결정 하나·명시적 다음 버튼·필수 메모·무저장 검토안 복사·새로고침 초기화·light/dark 보조·상태 문자 4.5:1·동일 origin GET·위험 버튼 비활성·320px 모든 결정 10,000px 이하·5화면+추적표 PNG·source SHA·clean 여부·소스/PNG digest manifest·mobile 표 헤더 접근성·desktop/mobile 가로 넘침 0. C0 gate 양성/음성 51건 | source `46a34ea`: [push CI 30975237262](https://github.com/ksi2564/systematic-trading/actions/runs/30975237262) 5/5 PASS · planning artifact `8918038132`; test-merge `992b403`: [PR CI 30975239146](https://github.com/ksi2564/systematic-trading/actions/runs/30975239146) 5/5 PASS · planning artifact `8918040822`. 두 ZIP과 source/PNG/UI 증적 SHA-256 독립 재검증 완료 | [QA-PLN-001 정본 증적](evidence/2026-08-05-QA-PLN-001.md) · [상세 QA 판정](TRACEABILITY_QA.md) | 부분 | D-01~D-10 검토 |
