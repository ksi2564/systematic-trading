...
class WeightSetTest {
    ...
    void drawDown_DD_퍼센트에_따른_기획서_비중표_일치(String ddPct, int qqq, int qld, int tqqq) {
        BigDecimal ddPercent = new BigDecimal(ddPct);

        WeightSet weights = WeightSet.drawDown(ddPercent);

        assertThat(weights.wQqq()).isEqualByComparingTo(Integer.toString(qqq));
    ...
    void of_팩토리_메서드로_커스텀_비중_생성() {
        ...
        assertThat(weights.wQqq()).isEqualByComparingTo("50.00");
        assertThat(weights.wQld()).isEqualByComparingTo("30.00");
