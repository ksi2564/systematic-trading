...
기준일: 2026-08-09 KST
상태: **C0-A 조건부 승인 완료 · C0-B 읽기 전용 수집 승인 · 결과 재승인 대기**
...
| Java를 멈췄나 | 서비스·PID는 중단하지 않았음. 실제 전략 on/off·최근 스케줄 성공·주문 모드는 승인된 C0-B 읽기 전용 범위에서 수집·정제 예정 |
| 지금 사용자가 할 일 | 정제된 12개 운영값·C0-A 차이·checksum을 본 뒤 C0-B 결과 재승인 여부 결정 |
...
| --- | --- | --- | --- | --- | --- |
| C0-B 읽기 전용 수집 승인 | 대상=12개; 접근방법=GitHub Actions PROD SSH로 운영 호스트 shell 조회·DB SELECT; 권한=읽기 전용; 정제=필수; 저장위치=docs/v2-cutover/evidence/c0b/; 원문저장=금지; 보존=Git 이력 | - | 수집 승인 | Inys | 2026-08-09T22:27:14+09:00 |
...
- 12개 개별 행이 모두 `확인 완료`가 되면 최종 bundle 행이 `재승인 대기`인 동안에도
  gate가 고정 경로의 `snapshot.json`과 `diff.json`, 모든 항목·세부 파일의 checksum과
  시각 순서를 검증한다. 최종 행은 사용자가 결과를 다시 승인하기 전까지 placeholder로 둔다.
