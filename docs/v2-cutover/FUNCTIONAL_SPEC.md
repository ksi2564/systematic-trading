...
| 안전 섀도 | 개발 대상 | 공식 데이터 읽기, 상태 평가, 주문 의도 계산, Java 대조, 증적 저장 | 브로커 주문 제출, Java 상태 변경 | C0-A/B 뒤 개발·로컬/격리 검증 자동, 공유 시험 서버 스케줄·데이터 생성은 실행 직전 사용자 승인 필수 |
...
이 절의 개발과 로컬·mock·일회용 격리 검증은 **C0-A/B 승인 이후**
승인된 전략·시점 기준 안에서 진행한다. 공유 시험 서버의 입력 조회, 스케줄 시작,
