...
이 문서 작성은 C2 개발 착수를 뜻하지 않는다. 현재 허용 범위는 코드·문서 읽기와 기획
정리뿐이며, C2 소스 변경은 `C0-A 확정 → C0-B 수집 별도 승인 → 정제된 운영값 수집·차이
확인 → C0-B 결과 재승인` 뒤에만 시작한다.
...
- 최근 20거래일의 세 레인, 실행 상세, QA 증적, 다음 승인
...
| --- | --- | --- | --- |
| C3 | 알고리즘·운영 결과·운영 준비 세 레인 | 연속 미국 거래일 20일 | 아니요. 공유 환경 실행과 예외 수용은 별도 승인 |
...
1. 검토 보드의 `실행 로드맵`에서 하루 흐름과 전체 전환 단계를 먼저 본다.
2. `결정 10가지`에서 D-01~D-06 전략·시간·데이터·수량·비교 방식을 정한다.
3. D-07에서 미래 QA 생성 범위·보존 기준만 정한다. 실행 승인은 아니다.
4. D-08~D-10에서 20거래일·복구·자동운용 완료 기준을 정한다.
5. 복사한 C0-A 검토안을 이 Codex 작업에 붙여넣는다.
6. 별도 C0-B 읽기 전용 수집 범위를 승인할지 판단한다.
7. 정제된 12개 값·차이·checksum을 본 뒤 C2 개발 착수를 재승인한다.
