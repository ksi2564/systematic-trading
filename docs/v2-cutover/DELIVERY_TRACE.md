...
- 이 결과를 연결하는 후속 증적 문구·기획 QA 단언 변경은 위 두 실행에 포함하지 않는다.
  위 두 실행을 해당 변경의 통과 근거로 쓰지 않고, 이를 담은 후속 커밋의 check를 별도
  증적으로 사용한다.
...
| D-07 | `V2-QA-001` | QA 기록 묶음·정리 tombstone·후보 전용 snapshot 기반 GET-only 배포 화면 harness | `S-03`, `S-08` | `QA-NAV-001`, `QA-OPS-001`, `QA-RWD-001`, `QA-SAF-001`: 로컬 가상 데이터 화면·증적 경계만 검증. 후속 후보의 `QA-ACC-002`는 exact health+snapshot 2개 backend GET, 5개 화면 API local fulfill/backend continue 0, 6화면×2 viewport의 PNG 12+관찰 JSON 2+manifest 1 계약만 로컬 검증했으며 실제 인증 화면은 미실행. 상태를 바꾸는 `QA-OPS-002`, `QA-MAN-001~009`도 미실행 | 기능 후보 `1520166`: clean-tree 26/26·증적 참조 78개 SHA-256 검증·리소스 변경 0과 [push CI 30959376884](https://github.com/ksi2564/systematic-trading/actions/runs/30959376884) artifact PASS; 당시 test-merge `5fca72e`: [PR CI 30959379072](https://github.com/ksi2564/systematic-trading/actions/runs/30959379072) artifact PASS. 이 두 실행은 후속 dirty 변경의 근거가 아니며 새 SHA CI가 필요. `MANUAL` 9·`BLOCKED` 2·`OUT_OF_SCOPE` 1은 통과로 올리지 않음 | [자동·수동 QA 경계](TRACEABILITY_QA.md#4-사용자에게-남길-수동-전용-시나리오) | 부분 | 후보 배포 사용자 승인 → 사용자 로그인 storage → 자동 GET-only QA |
...
| C1 현재 UI | `V2-UI-001`, `V2-OPS-001` | 현재 6개 화면, 안전 상태 `checking/verified/unsafe/unavailable` | 현재 운영 콘솔(S-01 일부·S-02·S-03·S-05·S-06·S-07) | `QA-NAV-001`, `QA-OPS-001`, `QA-SAF-001`, `QA-RWD-001`: **현재 구현 화면** desktop/mobile mock 후보. 배포 성공 bundle 계약은 6화면×2 viewport, PNG 12+관찰 JSON 2+manifest 1의 정확한 15파일 | 기능 후보 `1520166`: clean-tree 26/26·[push CI 30959376884](https://github.com/ksi2564/systematic-trading/actions/runs/30959376884) `ui-qa` PASS; 당시 test-merge `5fca72e`: [PR CI 30959379072](https://github.com/ksi2564/systematic-trading/actions/runs/30959379072) `ui-qa` PASS. 후속 변경은 새 SHA CI 미확인이고 운영 배포·인증 화면은 미실행 | [현재 UI QA 범위](TRACEABILITY_QA.md) | 부분 | 사용자 승인 동일 SHA 배포 → 인증 QA |
