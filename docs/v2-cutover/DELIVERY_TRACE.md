...
현재 게이트: D-01~D-10 C0-A는 `조건부 승인`, C0-B 12개 읽기 전용 수집은 `수집 승인`이다.
정제된 운영값·차이·checksum 수집과 결과 재승인은 남았고 C2는 `미시작`이다.
...
- 현재 C0-B 수집 승인 반영본: 로컬 C0 gate 58/58를 통과했다. 새 CI 통과 증적은 아직 없으며,
  후속 run이 실제로 완료되기 전에는 CI 성공을 미리 단정하지 않는다.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| D-01~02 · C0-B | `V2-STR-001`, `V2-STR-002`, `V2-DAT-001` | Python 평가기, Java/Python 공통 입력 비교, 미래 읽기 전용 exporter/comparator | `S-02`, `S-04`, `S-06`, `S-10` | `QA-PAR-001`: 공통 입력의 수식·경계·주문 종목·매수/매도 방향만 검증. 실제 운영값은 범위 밖 | 기능 후보 `1520166`: [push CI 30959376884](https://github.com/ksi2564/systematic-trading/actions/runs/30959376884) PASS; 당시 test-merge `5fca72e`: [PR CI 30959379072](https://github.com/ksi2564/systematic-trading/actions/runs/30959379072) PASS | [C0-B 수집 승인·미수집 항목](C0_DECISIONS.md#c0-b-운영-기준선-재확인-기록) | 부분 | 승인된 12개 읽기 전용 수집·정제 → 결과 재승인 |
