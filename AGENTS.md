...
- 작은 작업은 기능 단위로 세션을 마무리하고 handoff를 갱신한다.
- 큰 작업은 같은 기능 브랜치 안에서도 여러 세션으로 나눌 수 있다.
- 큰 작업에서 조사 완료, 변경 범위 확정, 1차 구현 완료, 테스트/PR 직전 같은 체크포인트에 도달하면 기능 완료 전이라도 handoff를 갱신할 수 있다.
- 컨텍스트 압축 위험이 커졌다고 판단되면 기능 완료 전이라도 handoff를 먼저 갱신하고 다음 세션으로 넘기는 것을 허용한다.
- handoff는 세션 종료 시점에만 쓰는 문서가 아니라, 다음 세션이 바로 이어받을 수 있게 만드는 중간 체크포인트 문서로도 사용한다.
